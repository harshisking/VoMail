        ...
        if 'exit' in command:
            ...
            if json_data['action'] == 'send_email':  # if send mail then send mail
                ...
                if json_data['command'] == "add_contact":
                    ...
                    if lis:
                        for c in lis:
                            print(f"ID: {c[0]} | Alias: {c[1]} | Email: {c[2]}")
                    pause()
